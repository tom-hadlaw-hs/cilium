...
var _ = Describe("K8sPolicyTest", func() {
	...
	Context("Basic Test", func() {
		...
		It("TLS policy", func() {
			...
			res := kubectl.CreateSecret("generic", "user-agent", namespaceForTest, "--from-literal=user-agent=CURRL")
			res.ExpectSuccess("Cannot create secret %s", "user-agent")

			res = kubectl.CreateSecret("generic", "test-client", namespaceForTest, "--from-file="+TLSCa)
			res.ExpectSuccess("Cannot create secret %s", "test-client")

			res = kubectl.CreateSecret("tls", "test-server", namespaceForTest, "--cert="+TLSCrt+" --key="+TLSKey)
